...
mod tests {
    ...
    fn output_is_equal_rounded(expected: [f32;MAX_BLOCK_SIZE], actual: [f32;MAX_BLOCK_SIZE], block_size: usize) {
        for i in 0..block_size {
            ...
            assert!(expected[i] - actual[i] < error_margin,
    ...
    fn multichannel_buffer_contains_value() {
        // Eight channels (MAX_CHANNEL_COUNT),
        // all initialized to the expected value.
        let expected = [
            [
        ...
        assert_eq!(expected, actual.channels,
            "The buffer was generated with the expected number of channels,\
            all of which contain the specified value. Actual: {:?}", actual);
    }
